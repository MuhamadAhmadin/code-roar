<template>
  <div class="overflow-hidden h-screen w-screen flex flex-col justify-center items-center relative">
    ...
    <div class="bg-gradient-to-tl from-yellow-700 via-orange-800 to-black-900 w-full h-full flex flex-col justify-center items-center transition-transform duration-100"
    ...
    <div class="fixed top-0 my-3 text-center">
      ...
      <form @submit.prevent="Login" class="w-full sm:w-1/2 md:w-1/2 lg:w-1/3 px-3 md:px-0 text-center">
        <Input @getvalue="(val) => usernameLogin = val" class="w-full h-12 md:h-16 text-lg" placeholder="Username"/>
        <Button color="orange" class="my-3 py-3 text-base md:text-xl w-full" type="submit">Login</Button>
      </form>
      <p class="text-white">belum punya akun ? <span @click="isDaftar = true" class="text-red-400 hover:text-red-500 cursor-pointer font-bold text-md">Daftar</span> </p>
    </div>

    <div class="bg-white  transition-transform transform bg-orange-200 duration-100 shadow-lg absolute right-0 top-0 w-full md:w-1/3" :class="isDaftar ? '' : 'translate-x-full'">
      ...
      <form @submit.prevent="Register" class="flex flex-col justify-center items-center h-screen">
        ...
        <Input @getvalue="(val) => usernameDaftar = val" placeholder="Username" id="register" class="my-3 w-1/2"/>
